...
COMMON_CFLAGS="-O3 -pipe -fno-caller-saves -fno-plt -fno-semantic-interposition -fgraphite-identity -floop-nest-optimize -fsched-pressure -fipa-pta -fno-conserve-stack"
TUNING_CFLAGS="-mcpu=cortex-a53+crypto+crc"
...
CONFIG_CCACHE=y
CONFIG_CCACHE_DIR="${CCACHE_DIR:-$(realpath $SCRIPT_DIR/.ccache)}"
...
if [ -n "${PREBUILT_GCC_VERSION:-}" ]; then
    echo "PREBUILT_GCC_VERSION is set to '$PREBUILT_GCC_VERSION'. Configuring toolchain..."
    if [ -d "$PREBUILT_GCC_VERSION" ]; then
        ./scripts/ext-toolchain.sh \
            --overwrite-config \
            --toolchain "$PREBUILT_GCC_VERSION" \
...
if [ "${DISTCLEAN_BUILD:-false}" = "true" ]; then
    generate_config
    make distclean
fi
